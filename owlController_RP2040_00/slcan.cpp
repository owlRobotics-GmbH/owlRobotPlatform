...
void SLCAN::begin(owlDriveCAN *aCanDriver) {
...
int SLCAN::b2ahex(char *p, uint8_t s, uint8_t n, void *v)
{
...
int SLCAN::a2bhex(char *p, uint8_t s, uint8_t n, void *v)
{
  int i, j;
  char buf[4+1];
  int val;
...
void SLCAN::onCanReceived(int id, int len, unsigned char canData[8])
//void SLCAN::xfer_can2tty()
{
  //tCAN msg;
  char buf[CMD_LEN];
  int i;
  ...
  //while (Canbus.message_rx(&msg)) {
  {
    ...
    /*if (msg.header.ide) {
      ...
      p += b2ahex(p, 4, 1, (uint16_t) &id);
      p += b2ahex(p, 4, 1, uint16_t &msg.ide);
      ...
      uint16_t msg_id = id; 
      p += b2ahex(p, 3, 1, &msg_id);      
      msg_len = len;
      p += b2ahex(p, 1, 1, &msg_len);
    }

    p += b2ahex(p, 2, len, canData);
...
void SLCAN::slcan_ack()
{
  Serial.write('\r'); // ACK
}

void SLCAN::slcan_nack()
{
  Serial.write('\a'); // NACK
}

void SLCAN::send_canmsg(char *buf)
{
  //tCAN msg;
  ...
  if (!is_eff && len >= 4) { // SFF
    a2bhex(&buf[1], 3, 1, msg_id); 
    a2bhex(&buf[4], 1, 1, &hlen);
    if (len - 4 - 1 == hlen * 2) {
      a2bhex(&buf[5], 2, hlen, msg_data);   
      //Serial.print("id=");
      //Serial.println(msg_id[0]);
      //Serial.print(" len=");
      //Serial.println(hlen);
      ...
      //}
      //Serial.println();
      canDriver->sendPacket(msg_id[0], hlen, msg_data);
...
void SLCAN::pars_slcancmd(char *buf)
{
  switch (buf[0]) {
    // common commands
    case 'O': // open channel
